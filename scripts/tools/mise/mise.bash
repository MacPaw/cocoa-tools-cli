...
mise_config_file() {
  MISE_CONFIG_FILE_LOCATIONS=(
    "${REPOSITORY_ROOT_DIR}/.config/mise/mise.toml"
    "${REPOSITORY_ROOT_DIR}/mise.toml"
    "${REPOSITORY_ROOT_DIR}/.mise.toml"
    "${REPOSITORY_ROOT_DIR}/.config/mise/config.toml"
  )

  MISE_CONFIG_FILE=""

  for file in "${MISE_CONFIG_FILE_LOCATIONS[@]}"; do
    if [ -f "${file}" ]; then
      MISE_CONFIG_FILE="${file}"
      break
    fi
  done
  ...
  if [ -z "${MISE_CONFIG_FILE}" ]; then
    for file in "${MISE_CONFIG_FILE_LOCATIONS[@]}"; do
      file_name="$(basename "${file}" .toml)"
      dir_name="$(dirname "${file}")"
      full_path="${dir_name}/${file_name}.local.toml"
      if [ -f "${full_path}" ]; then
        MISE_CONFIG_FILE="${full_path}"
        break
      fi
    done
  fi

  echo "mise config file: ${MISE_CONFIG_FILE}"
}
...
mise_install_dependencies() {
  mise_check_version

  echo "[ 🧰   mise ] Installing dependencies"
  "${MISE_BIN_PATH}" deps install --yes
}
...
while getopts "i:-:" OPTSPEC; do
  ...
  case "${OPTSPEC}" in
    ...
    *)
      ...
      echo "Available option: --install - install mise tools" >&2
      exit 1
      ;;
  esac
done
