...
container_run() {
  local SCRIPT_TO_RUN CONTAINER_WAS_RUNNING SWIFT_RELEASE_DIRECTORY
  SCRIPT_TO_RUN="${1}"
  ...
  # Apple's `container` requires host mount paths to exist; Docker often creates them.
...
container_run_tests() {
  container_run "./scripts/linux-container-actions/test.bash"
}

container_run_build() {
  container_run "./scripts/linux-container-actions/build.bash"
}
