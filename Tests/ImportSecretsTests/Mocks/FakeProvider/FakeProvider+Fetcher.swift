...
extension Fetcher: SecretFetcherProtocol {
  ...
  func fetchItem(_ item: Source.Item, keys: Set<String>, configuration: Source.Configuration) async throws -> [String: String] {

    fetchItemCalls.append((item: item, keys: keys, configuration: configuration))
    let missingKeyError = keys
      .filter { $0.hasSuffix("missing") }
      .map {
        FetchError.failedToFetch(keyMissing: "\(item.path)\($0)")
      }.first
