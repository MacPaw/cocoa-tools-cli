...
struct IntegrationTests {
  ...
  class MockURLProtocol: URLProtocol, @unchecked Sendable {
    ...
    override func startLoading() {
      ...
      guard let url = request.url?.absoluteString, let mockResponse = MockURLProtocol.mockResponses[url] else {
        let error = NSError(
          domain: "MockError",
          code: 404,
          userInfo: [
            NSLocalizedDescriptionKey: "Mock response not found",
            NSURLErrorKey: request.url ?? URL(fileURLWithPath: "/"),
          ]
        )
        client?.urlProtocol(self, didFailWithError: error)
        return
  ...
  func test_fetch_withKeyValueEngine_returnsSecrets() async throws {
    ...
    MockURLProtocol.setMockResponse(for: "https://vault.example.com/v1", data: Data(), statusCode: 200)
  ...
  func test_fetch_withAWSEngine_returnsCredentials() async throws {
    ...
    let configuration = try createMockConfiguration()
    let secrets = [
      "AWS_ACCESS_KEY": HashiCorpVaultReader.Element(aws: .init(enginePath: "aws", role: "my-role", key: "accessKey")),
      "AWS_SECRET_KEY": HashiCorpVaultReader.Element(aws: .init(enginePath: "aws", role: "my-role", key: "secretKey")),
    ]
    ...
    MockURLProtocol.setMockResponse(
      for: "https://vault.example.com/v1/aws/creds/my-role",
      data: """
        {
          "data": {
  ...
  func test_fetch_withMixedEngines_returnsAllSecrets() async throws {
    ...
    // Mock KeyValue secrets response
    MockURLProtocol.setMockResponse(
      for: "https://vault.example.com/v1/secret/data/myapp/database",
      data: """
        {
          "data": {
            "data": {
  ...
  func test_fetch_withHTTPError_throwsHTTPError() async throws {
    ...
    let secrets = [
      "DATABASE_PASSWORD": HashiCorpVaultReader.Element(
        keyValue: .init(secretMountPath: "secret", path: "myapp/database", version: 2, key: "password")
      )
    ]
    ...
    MockURLProtocol.setMockResponse(
      for: "https://vault.example.com/v1/secret/data/myapp/database?version=2",
      data: """
        {
  ...
  func test_fetch_withMissingSecretKey_throwsError() async throws {
    ...
    MockURLProtocol.setMockResponse(
      for: "https://vault.example.com/v1/secret/data/myapp/database?version=4",
      data: """
        {
          "data": {
            "data": {
  ...
  func test_authenticateWithAppRole_returnsToken() async throws {
    ...
    let configuration = HashiCorpVaultReader.Configuration(
      ...
      authenticationCredentials: .init(appRole: .init(roleId: "role-123", secretId: "secret-456")),
      authenticationMethod: .appRole
    )

    // Mock AppRole authentication response
    MockURLProtocol.setMockResponse(
      for: "https://vault.example.com/v2/auth/approle/login",
      data: """
        {
          "auth": "some",
  ...
  func test_authenticateWithAppRole_withInvalidResponse_throwsError() async throws {
    ...
    MockURLProtocol.setMockResponse(
      for: "https://vault.example.com/v1/auth/approle/login",
      data: """
        {
          "auth": "some"
        }
  ...
  func test_fetch_batchesRequestsForSameKeyValueItem() async throws {
    ...
    let secrets = [
      "DB_HOST": HashiCorpVaultReader.Element(
        keyValue: .init(secretMountPath: "secret", path: "myapp/database", version: 3, key: "host")
      ),
    ...
    MockURLProtocol.setMockResponse(
      for: "https://vault.example.com/v1/secret/data/myapp/database?version=3",
      data: """
        {
          "data": {
            "data": {
