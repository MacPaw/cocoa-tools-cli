...
extension SemanticVersion: LosslessStringConvertible {
  ...
  public enum ParseError: Swift.Error {
    ...
    public var errorDescription: String {
      ...
      case .invalidMajorVersion(let string): "Major version must be a non-negative integer, but got '\(string)'."
      case .invalidMinorVersion(let string): "Minor version must be a non-negative integer, but got '\(string)'."
      case .invalidPatchVersion(let string): "Patch version must be a non-negative integer, but got '\(string)'."
      case .invalidPrereleaseIdentifiers(let strings):
        "Prerelease identifiers must consist of ASCII alphanumerical characters except for '-'. Invalid identifiers: \(strings.joined(separator: ", "))"
      case .invalidBuildIdentifiers(let strings):
        "Build identifiers must consist of ASCII alphanumerical characters except for '-'. Invalid identifiers: \(strings.joined(separator: ", "))"
      }
    }
  }
...
extension SemanticVersion {
  ...
  package var codeExpression: String {
    ...
    if !buildMetadataIdentifiers.isEmpty {
      params.append(
        "buildMetadataIdentifiers: [\(buildMetadataIdentifiers.map { "\"\($0)\"" }.joined(separator: ", "))]"
      )
    }

    return "SemanticVersion(\(params.joined(separator: ", ")))"
  }
}
